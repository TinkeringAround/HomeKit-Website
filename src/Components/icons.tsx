...
export const AC: FC<Props> = ({ color, width = '80%', height = '80%' }) => (
  <svg id="icon-ac" viewBox="0 0 39 32" height={height} width={width}>
    <title>ac</title>
    <path
      fill={color}
      style={{ fill: color }}
...
export const Humidity: FC<Props> = ({ color, width = '80%', height = '80%' }) => (
  <svg id="icon-humidity" viewBox="0 0 39 32" width={width} height={height}>
    ...
    />
    <path stroke={color} style={{ stroke: color }} d="M10.508 20.36c0 0 0.933 6.18 7.233 7.653" />
  </svg>
)
...
export const Light: FC<Props> = ({ color, width = '80%', height = '80%' }) => (
  <svg id="icon-light" viewBox="0 0 39 32" width={width} height={height}>
    ...
    />
    <path fill={color} style={{ fill: color }} d="M15.615 24.455h7.636v1.091h-7.636v-1.091z" />
    <path fill={color} style={{ fill: color }} d="M15.615 27.182h7.636v1.091h-7.636v-1.091z" />
    <path fill={color} style={{ fill: color }} d="M17.251 29.909h4.364v1.091h-4.364v-1.091z" />
  </svg>
)

export const Sensor: FC<Props> = ({ color, width = '80%', height = '80%' }) => {
  return (
    <svg id="icon-sensor" viewBox="0 0 39 32" width={width} height={height} fill={color}>
      <title>sensor</title>
      <path d="M19.419 14.543c0.851 0 1.541 0.651 1.541 1.457 0 0.799-0.69 1.457-1.541 1.457s-1.541-0.651-1.541-1.457c0-0.799 0.69-1.457 1.541-1.457zM19.419 13.254c-1.566 0-2.83 1.225-2.83 2.746 0 1.515 1.27 2.746 2.83 2.746s2.83-1.231 2.83-2.746c0.006-1.515-1.263-2.746-2.83-2.746v0z" />
      <path d="M24.518 10.785c1.399 1.334 2.269 3.178 2.269 5.215s-0.864 3.874-2.263 5.209" />
      <path d="M28.482 6.95c2.45 2.314 3.964 5.518 3.964 9.050s-1.515 6.736-3.964 9.050" />
      <path d="M32.453 3.108c3.5 3.3 5.666 7.858 5.666 12.892s-2.166 9.592-5.666 12.892" />
      <path d="M14.32 21.209c-1.399-1.334-2.269-3.178-2.269-5.215s0.864-3.874 2.263-5.208" />
      <path d="M10.381 25.050c-2.45-2.314-3.964-5.518-3.964-9.050s1.515-6.736 3.964-9.050" />
      <path d="M6.391 28.892c-3.5-3.3-5.666-7.858-5.666-12.892s2.166-9.592 5.666-12.892" />
    </svg>
  )
}
...
export const Temperature: FC<Props> = ({ color, width = '80%', height = '80%' }) => (
  <svg id="icon-temp" viewBox="0 0 39 32" width={width} height={height}>
    ...
    />
    <path fill={color} style={{ fill: color }} d="M9.755 2.963h10.076v1.185h-10.076v-1.185z" />
    ...
    <path fill={color} style={{ fill: color }} d="M9.755 11.854h10.076v1.185h-10.076v-1.185z" />
    <path fill={color} style={{ fill: color }} d="M12.718 14.818h7.112v1.185h-7.112v-1.185z" />
    <path fill={color} style={{ fill: color }} d="M12.718 17.781h7.112v1.185h-7.112v-1.185z" />
    <path fill={color} style={{ fill: color }} d="M9.755 20.745h10.076v1.185h-10.076v-1.185z" />
  </svg>
)
...
export const Window: FC<Props> = ({ color, width = '80%', height = '80%' }) => (
  <svg id="icon-window" viewBox="0 0 39 32" width={width} height={height}>
    ...
    />
    <path fill={color} style={{ fill: color }} d="M18.755 12.667h1.333v18.667h-1.333v-18.667z" />
    <path fill={color} style={{ fill: color }} d="M31.422 21.333v1.333h-25.333v-1.333h25.333z" />
  </svg>
)
